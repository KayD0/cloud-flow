...
export default function SiteToSiteVpnConnectivityPage() {
  ...
  return <main><SiteHeader /><div className="page-shell detail-shell">
    ...
    <header className="category-hero template-detail-hero"><p className="hero-kicker"><span /> INTERACTIVE TEMPLATE</p><h1>{template.name}</h1><p>{template.summary}</p><ul className="tag-list" aria-label="補助タグ">{template.tags.map((tag) => <li key={tag}>{tag}</li>)}</ul></header>

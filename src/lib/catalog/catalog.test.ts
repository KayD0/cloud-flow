...
describe("template catalog", () => {
  ...
  it("places available categories before planned categories", () => {
    ...
    const firstPlanned = categories.findIndex((category) => !category.isAvailable);
    expect(firstPlanned).toBeGreaterThan(0);
    expect(categories.slice(0, firstPlanned).every((category) => category.isAvailable)).toBe(true);
    expect(categories.slice(firstPlanned).every((category) => !category.isAvailable)).toBe(true);
  });
